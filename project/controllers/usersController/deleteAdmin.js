		...
		if (req.user.AccessLevel === 1) {
			...
			if (!userExist) {
				return res.status(404).send({
					message: "There are no entries with such 'User ID'."
				})
			} else if (userExist.AccessLevel === 1) {
				return res.status(403).send({
					message: "You can't delete superuser. Please contact technical administrator."
				});
			} else {
				const deletedUser = await UsersModel.findByIdAndDelete(userId);
				await loggerModule(`Користувач '${deletedUser.Fullname}' успішно видалений`, req.user.Login);
				return res.status(200).send({
					message: `User '${deletedUser.Fullname}' successfully deleted.`
				});
			}
		} else {
